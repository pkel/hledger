...
--
-- Note that setWithReport/overWithReport should only be necessary with
-- querystring and reportOpts: the other lenses should never fail.
--
-- === Examples:
-- >>> import Lens.Micro (set)
-- >>> _rsQuery <$> setWithReport querystring ["assets"] defreportspec
...
-- >>> _rsQuery $ set querystring ["(assets"] defreportspec
-- *** Exception: Updating ReportSpec failed: try using overWithReport instead of over or setWithReport instead of set
...
instance HasReportOpts ReportSpec where
    reportOpts f rspec = report (error' "Updating ReportSpec failed: try using overWithReport instead of over or setWithReport instead of set") $  -- PARTIAL:
